...
'Use Strict';
angular.module('App').controller('attendanceLogsController', function($scope, HoursCalc, Service, Watchers , $ionicModal, $timeout,$filter, $http, $rootScope, $http, $ionicSideMenuDelegate, $rootScope, $state, $localStorage, Utils, Popup, $timeout, Service, Watchers, $ionicTabsDelegate, $ionicHistory) {
  ...
  $scope.$on('$ionicView.enter', function() {
	...
	firebase.database().ref('logs/' + empId).once('value', function(accountName) {
		...
		var cnt=0;
		for(var i=logTime.logTime.length-1;i>0;i--)
		{
			...
			if(logType=="In")
			{
				...
				if(cnt==2)
				{
					...
					console.log();
					$scope.logDay=$filter('date')(new Date(logTime.logTime[i].time), 'EEEE');  
					$scope.logDate=$filter('date')(new Date(logTime.logTime[i].time), 'd');
					$scope.logMonth=$filter('date')(new Date(logTime.logTime[i].time), 'MMMM');
					$scope.logYear=$filter('date')(new Date(logTime.logTime[i].time), 'yy');
					$scope.logTime=$filter('date')(new Date(logTime.logTime[i].time), 'h:mm a');
					$scope.logLocation=logTime.logTime[i].location;
					break;
				}
